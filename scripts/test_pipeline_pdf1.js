...
async function testPipeline() {
  ...
  const data = new Uint8Array(fs.readFileSync('/Users/akshayryali/Downloads/90-90-989 Rev H Manual-pages/90-90-989 Rev H Manual-pages-1.pdf'));
  const doc = await pdfjsLib.getDocument({data}).promise;
  const page = await doc.getPage(171);
  const content = await page.getTextContent();
  const text = content.items.map(item => item.str).join(" ");
  console.log("TEXT PREVIEW PAGE 171:");
  console.log(text.substring(0, 300));
  
  let prompt = appJsCode.match(/const systemPrompt = `(.*?)`;/s)[1];
  prompt = prompt.replace('${cleanDocName}', '90-90-989 Rev H Manual-pages-1');
  ...
  console.log(JSON.stringify(JSON.parse(json.response), null, 2));
}
