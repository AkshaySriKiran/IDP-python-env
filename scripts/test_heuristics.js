...
function isRecommendedSparePartsPage(pageText) {
  ...
  if (/\.{4,}/.test(pageText) || /\.\s*\.\s*\.\s*\.\s*\./.test(pageText) || pageText.toLowerCase().includes("table of contents")) return false;
...
function parseSparePartsStructurally(text, docName, pageNum) {
    return [];
}
...
function runRuleExtractorHeuristics(text, docName, pageNum = 1) {
  if (isRecommendedSparePartsPage(text)) {
    ...
    return { maintenance: [], spare_parts: spareParts };
  }
...
async function run() {
  ...
  for(let i=1; i<=doc.numPages; i++) {
     ...
     const result = runRuleExtractorHeuristics(text, "LUBE OIL PUMP  SEIM-MANUAL (1).pdf", i);
     allMaint.push(...result.maintenance);
  }
  console.log("Total extracted:", allMaint.length);
